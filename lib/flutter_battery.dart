...
class FlutterBattery {
  ...
  Stream<BatteryInfo> get batteryInfoStream {
    return batteryStream.where((event) {
      ...
      return BatteryInfo(
        ...
        temperature:
            (event[BatteryPayloadKeys.temperature] as num?)?.toDouble() ?? 0.0,
  ...
  void configureBatteryCallbacks({
    ...
    if (onBatteryInfoChange != null) {
      FlutterBatteryPlatform.instance
          .setBatteryInfoChangeCallback((Map<String, dynamic> infoMap) {
    ...
    if (onBatteryHealthChange != null) {
      FlutterBatteryPlatform.instance
          .setBatteryHealthChangeCallback((Map<String, dynamic> map) {
  ...
  void setBatteryLevelChangeListener(
    ...
    FlutterBatteryPlatform.instance
        .setBatteryLevelChangeCallback(onBatteryLevelChanged);
  }
  ...
  Future<Map<String, bool>> configureBatteryMonitor(
      BatteryMonitorConfig config) async {
  ...
  Future<Map<String, dynamic>> configureBattery(
      BatteryConfiguration config) async {
    ...
    if (config.monitorConfig != null) {
      result['monitoringResults'] =
          await configureBatteryMonitor(config.monitorConfig!);
    }

    // 3. 配置低电量监控
    if (config.lowBatteryConfig != null) {
      result['lowBatteryMonitoring'] =
          await configureBatteryMonitoring(config.lowBatteryConfig!);
    }
